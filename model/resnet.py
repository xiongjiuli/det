...
class Decoder(nn.Module):
    ...
    def _make_conv_layers(self, inplanes, num_layers=5, init_filters=256): #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!修改了num_layers = 5
        in_c = inplanes
        out_c = init_filters
        layers = []
...
class CenterNet(nn.Module):
    ...
    def forward(self, x):
        # embed()
        x = self.backbone(x)
        # embed() #* if the input is torch.randn(1, 1, 512, 512, 256), the backbone is torch.Size([1, 2048, 8, 8, 8])
...
if __name__ == '__main__':
    ...
    a = torch.randn(1, 1, 256, 256, 256)
    # a = torch.randn(1, 1, 256, 256, 256)
    model = CenterNet('resnet101', 1)
    hmap, whd, offset = model(a)
    print(f'the hmap.shape is : {hmap.shape}, the whd.shape is : {whd.shape}, the offset.shape is : {offset.shape}')
    # embed()
