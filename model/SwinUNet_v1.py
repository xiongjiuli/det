...
class PatchEmbed3D(nn.Module):
    ...
    def forward(self, x):
        ...
        print(f'the x.shape (in embedding) is :{x.shape}') # torch.Size([1, 4, 128, 128, 128])
        x = self.proj(x)  # B C D Wh Ww
        print(f'after the proj(x), the x.shape is : {x.shape}, and the x.size() is : {x.size()}') # torch.Size([1, 96, 32, 32, 32]), torch.Size([1, 96, 32, 32, 32])
        if self.norm is not None:
            D, Wh, Ww = x.size(2), x.size(3), x.size(4)
            x = x.flatten(2).transpose(1, 2)
            print(f'after the x = x.flatten(2).transpose(1, 2), the x.shape is : {x.shape}') # torch.Size([1, 32768, 96])
            x = self.norm(x)
            print(f'after the x = self.norm(x), the x.shape is : {x.shape}') # torch.Size([1, 32768, 96])
            x = x.transpose(1, 2).view(-1, self.embed_dim, D, Wh, Ww)
            print(f'after the x = x.transpose(1, 2).view(-1, self.embed_dim, D, Wh, Ww), the x.shape is : {x.shape}') # torch.Size([1, 96, 32, 32, 32])
...
class SwinTransformerSys3D(nn.Module):
    ...
    def forward_features(self, x):
        print(f'the input x.shape is : {x.shape}')
        x = self.patch_embed(x)
        print(f'after the self.patch_embed(x), the x.shape is : {x.shape}')
        x = self.pos_drop(x)
        print(f'after the self.pos_drop(x), the x.shape is : {x.shape}')
        x_downsample = []
        v_values_1 = []
        k_values_1 = []
        q_values_1 = []
    ...
    def forward(self, x):
        x, x_downsample, v_values_1, k_values_1, q_values_1, v_values_2, k_values_2, q_values_2 = self.forward_features(
            x)
        # embed()
        print(f'after the self.forward_features, the x shape is {x.shape}, the x_downsample shape is {x_downsample[0].shape}, \
              the v_values_1 shape is {v_values_1[0].shape}, the k_values_1 shape is {k_values_1[0].shape}, \
              the q_values_1 shape is {q_values_1[0].shape}, the v_values_2 shape is {v_values_2[0].shape}, \
              the k_values_2 shape is {k_values_2[0].shape}, the q_values_2 shape is {q_values_2[0].shape}')

        x = self.forward_up_features(x, x_downsample, v_values_1, k_values_1, q_values_1, v_values_2, k_values_2,
                                     q_values_2)
        print(f'before the self.up_x4 is {x.shape}')
        x = self.up_x4(x)
        print(f'after the self.up_x4 is {x.shape}')
...
if __name__ == '__main__':
    ...
    swin_unet = SwinTransformerSys3D(in_chans=1)
    x1 = torch.randn(1, 4, 512, 512, 256)

    x2 = torch.randn(1, 1, 128, 128, 128)
    x3 = torch.randn(1, 1, 256, 256, 256)
    y = swin_unet(x3)
