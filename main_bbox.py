...
def train(config):
    ...
    train_loader = data.DataLoader(train_dataset, batch_size=config.train_batch_size, shuffle=True, num_workers=4, drop_last=True)
    valid_loader = data.DataLoader(valid_dataset, batch_size=config.valid_batch_size, shuffle=False, num_workers=4)
    ...
    if config.model != 'normal':
        model_path = 'D:\Work_file\det\save\model-19.pt'
    ...
    with tqdm(total=(config.total_steps - 20)) as pbar:
        for step in range(20, config.total_steps):
            ...
            # if (step) % 100 == 0:
                ...
                with torch.no_grad():
                    ...
                    for valid_batch in tqdm(valid_loader):
                        ...
                        name = valid_batch['name']
                        hmap, whd, offset = model(image)
                        # embed()
                        hmap_loss = focal_loss(hmap, heatmap, config.point_weight)

                        # predicted_boxes = decode_bbox(hmap, whd, offset, confidence=4, reduce=4, cuda=None)
                        # ground_truth_boxes = read_and_convert_data(name)
                        # accuracy, recall = calculate_accuracy_and_recall(predicted_boxes, ground_truth_boxes, iou_threshold=0.5)
            ...
            for train_batch in tqdm(train_loader):
                ...
                loss.backward()
                optimizer.step()
                logger.info('Epoch: %d, Batch_step: %d, train_Loss: %.4f', step, batch_step, loss.item())
                batch_step += 1
            # print(loss.item())
