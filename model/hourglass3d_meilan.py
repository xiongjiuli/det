...
class convolution(nn.Module):
  def __init__(self, k, inp_dim, out_dim, stride=1, with_bn=True):
    ...
    pad = (k - 1) // 2
    self.conv = nn.Conv3d(inp_dim, out_dim, k, padding=pad, stride=stride, bias=not with_bn)
...
class residual(nn.Module):
  def __init__(self, k, inp_dim, out_dim, stride=1, with_bn=True):
    ...
    self.conv1 = nn.Conv3d(inp_dim, out_dim, 3, padding=1, stride=stride, bias=False)
    self.bn1 = nn.BatchNorm3d(out_dim)
    self.relu1 = nn.ReLU(inplace=True)

    self.conv2 = nn.Conv3d(out_dim, out_dim, 3, padding=1, bias=False)
    self.bn2 = nn.BatchNorm3d(out_dim)

    self.skip = nn.Sequential(nn.Conv3d(inp_dim, out_dim, 1, stride=stride, bias=False),
...
class exkp(nn.Module):
  def __init__(self, n, nstack, dims, modules, in_c = 1, cnv_dim=256, num_classes=80):
    ...
    self.inters_ = nn.ModuleList([nn.Sequential(nn.Conv3d(curr_dim, curr_dim, 1, bias=False),
                                                nn.BatchNorm3d(curr_dim))
                                  for _ in range(nstack - 1)])
    self.cnvs_ = nn.ModuleList([nn.Sequential(nn.Conv3d(cnv_dim, curr_dim, 1, bias=False),
                                              nn.BatchNorm3d(curr_dim))
                                for _ in range(nstack - 1)])
    # heatmap layers
    ...
    # regression layers
    self.regs = nn.ModuleList([make_kp_layer(cnv_dim, curr_dim, 3) for _ in range(nstack)])
    self.w_h_ = nn.ModuleList([make_kp_layer(cnv_dim, curr_dim, 3) for _ in range(nstack)])
...
get_hourglass = \
  {'large_hourglass':
     exkp(n=5, nstack=2, dims=[256, 256, 384, 384, 384, 512], modules=[2, 2, 2, 2, 2, 4], num_classes=1, in_c=1),
   'small_hourglass':
     exkp(n=5, nstack=1, dims=[256, 256, 384, 384, 384, 512], modules=[2, 2, 2, 2, 2, 4], num_classes=1, in_c=1)}


if __name__ == '__main__':
  from IPython import embed
  model = get_hourglass['large_hourglass']
  image = torch.randn(1, 1, 256, 256, 256)
  outs = model(image)[-1]
  print(model(image)[0][0].shape)
  embed()